...
class TestCRUDOperations:
    ...
    async def test_invoice_creation_with_product_lines(self, client):
        """Test invoice creation passes lines through without transformation.

        The tool layer (not the client) handles lines individually via
        add_invoice_line, which maps product_id → fk_product.
        """
        with patch.object(client, 'request') as mock_request:
            mock_request.return_value = {"id": 101}

            # Create invoice with lines - client passes them through as-is
            ...
            # Verify payload is passed through without transformation
            ...
            # Lines are passed through unchanged by the client
            assert lines[0]["product_id"] == 10
