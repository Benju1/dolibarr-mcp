...
class DolibarrClient:
    ...
    async def download_document(self, modulepart: str, original_file: str) -> Dict[str, Any]:
        """Download a document already generated by Dolibarr.

        original_file is the relative path within the module's document
        directory (e.g. "PR2601-0001/PR2601-0001.odt"), typically taken from
        an object's `last_main_doc` field. Returns filename/content-type/
        filesize/content (base64-encoded)/encoding.
        """
        params = {"modulepart": modulepart, "original_file": original_file}
        return await self.request("GET", "documents/download", params=params)
    ...
    async def build_document(
        ...
        """(Re)generate a document server-side (e.g. after editing a proposal).

        original_file only needs to resolve to the object's reference
        (Dolibarr strips the extension to look up the record), so any
        extension works regardless of the actual template output format.
        """
        payload: Dict[str, Any] = {"modulepart": modulepart, "original_file": original_file}
        if doctemplate:
            payload["doctemplate"] = doctemplate
        if langcode:
            payload["langcode"] = langcode
        return await self.request("PUT", "documents/builddoc", data=payload)
